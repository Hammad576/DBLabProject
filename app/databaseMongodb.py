...
class Database:
    ...
    def insertStudent(
        ...
        return self.coll.insert_one({
            'gender': value1,
            'region': value2,
            'highest_education': value3,
            'imd_band': value4,
            'age_band': value5,
            'num_of_prev_attempts': value6,
            'is_banked': value7,
            'code_module_x': value8,
            'code_presentation_x': value9,
            'code_module_y': value10,
            'code_presentation_y': value11,
    ...
    def prediction(self):
        try:
            w_male = self.coll.find({'gender': 1,
                                    'prediction': 'Withdrawn'}).count()
            w_female = self.coll.find({'gender': 0,
                    'prediction': 'Withdrawn'}).count()

            p_male = self.coll.find({'gender': 1, 'prediction': 'Pass'
                                    }).count()
            p_female = self.coll.find({'gender': 0, 'prediction': 'Pass'
                    }).count()

            f_male = self.coll.find({'gender': 1, 'prediction': 'Fail'
                                    }).count()
            f_female = self.coll.find({'gender': 0, 'prediction': 'Fail'
                    }).count()

            d_male = self.coll.find({'gender': 1,
                                    'prediction': 'Distinction'
                                    }).count()
            d_female = self.coll.find({'gender': 0,
                    'prediction': 'Distinction'}).count()
        except:
